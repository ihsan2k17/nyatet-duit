...
export class SellTransactionReksadanaUsecase {
    ...
    async Execute(model: RDTransactionModel):Promise<Result<void>> {
        try{
            ...
            if(!entity.isSell()) {
                return Result.error("Invalid transaction type: not Sell ")
            }
            return await this.repo.AddSellReksadanaTrn(entity.toModel())
        } catch (error: unknown) {
